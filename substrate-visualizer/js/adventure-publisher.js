/* ============================================================================
   ADVENTURE PUBLISHER
   Generates self-contained HTML files for published adventures
   ============================================================================ */

const AdventurePublisher = (() => {
  ...
  function generateHTML(adventure, scenes, options = {}) {
    const {
      includeSpeedControl = true,
    ...
    <div id="end-overlay" style="display: none;">
      <div class="end-content">
        ...
        <div class="end-actions">
          ${includeRestartButton ? '<button id="btn-restart" class="action-btn primary">Restart</button>' : ''}
        </div>
      </div>
    </div>
...
// ============================================================================
// STICK SCENE PLAYER (inlined)
// ============================================================================

${getStickScenePlayerCode()}

// ============================================================================
// ADVENTURE PLAYER (simplified for standalone)
// ============================================================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Get minimal CSS for standalone playback
...
`;
  ...
  // Get the StickScenePlayer code for inlining
  function getStickScenePlayerCode() {
    // Return a minified/self-contained version
    return `
  ...
  class Player {
    ...
    _registerCoreSkills() {
      ...
      this.registerSkill("typeCode", async (ctx, step) => {
        ...
        for (let i = 0; i < text.length; i++) { editor.text += text[i]; editor.el.textContent = editor.text; editor.el.appendChild(cursor); await sleep(1000 / speedCps); }
      ...
      this.registerSkill("writeBoard", async (ctx, step) => {
        ...
        for (let i = 0; i < text.length; i++) { board.text += text[i]; board.el.textContent = board.text; await sleep(1000 / speedCps); }
...
`;
  ...
  // Get the AdventurePlayer code for inlining (simplified for standalone)
...
const AdventurePlayer = (() => {
  ...
  let stageEl, choiceOverlay, choicePrompt, choiceOptions, endOverlay, endSummary, stepCounterEl, pauseBtnEl, speedSelectEl, restartBtnEl;
...
`;
  ...
  // Trigger download of the published file
  ...
  async function publish(adventureId, options = {}) {
    const adventure = AdventureManager.getAdventure(adventureId);
    if (!adventure) {
      console.error('AdventurePublisher: Adventure not found:', adventureId);
      return false;
    }

    // Collect all scenes referenced by this adventure
    const scenes = collectScenes(adventure);

    // Generate the HTML
    const html = generateHTML(adventure, scenes, options);

    // Trigger download
    const filename = `${adventure.id || 'adventure'}.html`;
    downloadFile(filename, html);

    console.log(`AdventurePublisher: Published "${adventure.title}" (${Object.keys(scenes).length} scenes)`);
    EventBus.emit('adventure:published', { adventureId, filename, sceneCount: Object.keys(scenes).length });
  ...
  return {
    publish,
    collectScenes,
    generateHTML // Exposed for testing
  };
})();
